...
/*--
controller_server의 가장 상위에서 실행되는 메소드이다.
로봇의 cmd_vel을 계산하여 발행한다.
*/
...
/*--
odom 토픽으로부터 로봇의 현재 속도를 인지하고,
'velocity_threshold' 값 이하의 값은 잡음이라 생각하여 잘라내고 사용한다.
*/
...
/*--
'Follow_Path' controller의 computeVelocityCommands() 메소드를 실행한다.
현재 'Follow_Path'는 dwb plugin으로 설정되어 있다.
현재 로봇의 cmd_vel을 아규먼트로 넘겨주고, dwb plugin에 의해 계산된 로봇의 다음 최적 cmd_vel을 반환받는다.
*/
    cmd_vel_2d =
...
/*--
dwb plugin에 의해 계산된 로봇의 다음 최적 cmd_vel을 publish 한다.
*/
